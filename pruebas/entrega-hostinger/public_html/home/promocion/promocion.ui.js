...
export class PromocionUI {
  ...
  busy(busy) { const b = this.$('#chapi-promo-spin'); b.disabled = busy; b.setAttribute('aria-busy', String(busy)); b.firstElementChild.textContent = busy ? 'Descubriendo tu próximo plan…' : 'Girar ruleta'; this.text('#chapi-promo-spin-status', busy ? 'Estamos preparando tu premio.' : 'Sin registro. Solo buenos planes.'); }
  renderState(state, redraw = true) {
    ...
    this.text('#chapi-promo-unavailable-message', state.negocios.length ? 'No tienes un giro disponible en este momento. Puedes conseguir otra oportunidad con visitas referidas válidas o cuando un aliado valide uno de tus códigos.' : 'Los aliados están preparando sus promociones. Conservamos tu oportunidad y tu progreso para cuando estén disponibles.');
  }
  renderPrize(p) {
    ...
    this.text('#chapi-promo-expiry', `Vence: ${new Intl.DateTimeFormat('es-CO', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'America/Bogota' }).format(new Date(p.vence_at))} · Bogotá`);
    const claim = this.$('#chapi-promo-claim'); const valid = p.estado === 'activo' && /^[1-9][0-9]{7,14}$/.test(p.whatsapp);
    if (valid) claim.href = `https://wa.me/${p.whatsapp}?text=${encodeURIComponent(`Hola, vengo de Chapitour.co. Mi promoción en ${p.negocio} es ${p.titulo}. Mi código es ${p.codigo}.`)}`; else claim.removeAttribute('href');
    claim.setAttribute('aria-disabled', String(!valid)); this.$('#chapi-promo-history').value = p.id; this.tick();
  }
  tick() { if (!this.prize) return; const seconds = Math.max(0, Math.ceil((Date.parse(this.prize.vence_at) - Date.now() - this.clockOffset) / 1000)); this.text('#chapi-promo-countdown', this.prize.estado === 'redimido' ? 'Redimido' : !seconds ? 'Vencido' : [Math.floor(seconds / 3600), Math.floor(seconds % 3600 / 60), seconds % 60].map(n => String(n).padStart(2, '0')).join(':')); if (!seconds || this.prize.estado !== 'activo') { this.$('#chapi-promo-claim').removeAttribute('href'); this.$('#chapi-promo-claim').setAttribute('aria-disabled', 'true'); } }
}
