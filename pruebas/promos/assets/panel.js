...
async function load() {
  ...
  if (selectedPrize) { const result = await makeAjaxRequest('consultar_codigo', { codigo: selectedPrize.codigo }); showPrize(result.premio); }
}
function render(data) {
  ...
  $('#prize-rows').replaceChildren(...data.premios.map(p => {
    ...
    const first = el('td'); first.append(code); const business = el('td', p.negocio); business.append(el('small', p.titulo), el('small', p.direccion || 'Dirección por confirmar'), el('small', `${p.vigencia_horas} horas para redimir`)); const state = el('td'); state.append(badge(p.estado)); row.append(first, business, state, el('td', date(p.creado_at)), el('td', date(p.vence_at)), el('td', date(p.redimido_at))); return row;
  }));
...
function showPrize(prize) {
  selectedPrize = prize; const box = $('#lookup-result'); box.hidden = false; box.className = ''; box.replaceChildren(prizeCard(prize));
  box.append(el('small', `Aperturas de WhatsApp: ${prize.whatsapp_aperturas}. Abrir el enlace no confirma el envío ni redime el código.`));
  $('#redeem-form').hidden = prize.estado !== 'activo' || currentUser.rol !== 'aliado'; $('#redeem-form').reset();
  $('#owner-redemption-note').hidden = currentUser.rol === 'aliado';
}
...
setInterval(() => { if (currentUser && !document.hidden && !document.querySelector('dialog[open]') && !$('#dashboard').hidden) load().catch(handle); }, 30000);
document.addEventListener('visibilitychange', () => { if (!document.hidden && currentUser && !$('#dashboard').hidden) load().catch(handle); });
